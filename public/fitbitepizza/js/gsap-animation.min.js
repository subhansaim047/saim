...
ScrollTrigger.matchMedia({"(min-width: 1px)":function(){pbmit_tween.forEach((box,i)=>{let toppos=box.getAttribute('data-start-position');let tl=gsap.timeline({scrollTrigger:{trigger:box,start:"top "+toppos+"",end:"bottom 70%",scrub:1},defaults:{ease:"none"}});
    ...
    gsap.set(box
,{xPercent:xpos_val,yPercent:ypos_val,scale:scale_x_val,skewX:skew_x_val,skewY:skew_y_val,rotation:rotation_x_val});tl.to(box,{xPercent:xpose_val,yPercent:ypose_val,scale:scale_xe_val,skewX:skew_xe_val,skewY:skew_ey_val,rotation:rotation_xe_val})})},})}
...
ScrollTrigger.addEventListener("refresh",pbmit_title_animation);jQuery(document).ready(function(){pbmit_staticbox_hover()});jQuery(window).load(function(){pbmit_title_animation();pbmit_hover_img();pbmit_tween_effect();pbmit_img_animation();pbmit_sticky();pbmit_bg_change();pbmit_sticky_menu();pbmit_horizontal_snap_section();const cursor=new Cursor();jQuery('[data-magnetic]').each(function(){new Magnetic(this)});gsap.delayedCall(1,()=>ScrollTrigger.getAll().forEach((t)=>{t.refresh()}))})